...
exports.movie_create_post = async (req, res) => {
  upload(req, res, async (err) => {
    ...
    // Retrieve title
    const title = Array.isArray(req.body.title) ? req.body.title[0] : req.body.name;
    ...
    try {
      ...
      const videoResponse = await axios.get(`https://api.themoviedb.org/3/movie/${movieData.id}/videos`, {
      ...
      const localPosterPath = path.join(__dirname, '../public/images', posterFileName);
...
exports.movie_details = async (req, res) => {
  ...
  try {
    ...
    movieObj.poster = movie.poster_path ? `https://image.tmdb.org/t/p/w500${movie.poster_path}` : null; 
